...
from diffusers_helper.utils import write_PIL_image_with_png_info
...
quick_prompts_path = os.path.join(os.path.dirname(__file__), "quick_prompts.txt")
with open(quick_prompts_path, encoding="utf-8") as f:
    # each line in quick_prompts.txt should be a single prompt
    quick_prompts = [[line.strip()] for line in f if line.strip()]
...
def worker(input_image, end_frame, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, save_section_frames, keep_section_videos, output_dir=None, section_settings=None, use_lora=False, lora_file=None, lora_scale=0.8, lora_format="HunyuanVideo", end_frame_strength=1.0, use_all_padding=False, all_padding_value=1.0):
    ...
    try:
        ...
        # Encode all current settings into PNG text chunks
        metadata = {
            'prompt':                  prompt,
            'seed':                    str(seed),
            'total_length_sec':        str(total_second_length),
            'latent_window_size':      str(latent_window_size),
            'steps':                   str(steps),
            'cfg_scale':               str(cfg),
            'gpu_mem_preserve_gb':     str(gpu_memory_preservation),
            'use_teacache':            str(use_teacache),
            'lora':                    str(use_lora),
            'lora_file':               str(lora_file),
            'lora_scale':              str(lora_scale),
            'lora_format':             str(lora_format),
            'end_frame_strength':      str(end_frame_strength),
            'use_all_padding':         str(use_all_padding),
            'all_padding_value':       str(all_padding_value),
            # add any other settings you want to capture here…
        }
        ...
        for i_section, latent_padding in enumerate(latent_paddings):
            ...
            # 定義後にログ出力
            print(f"\n\u25a0 Processing section {i_section} has started (" + (f"Set padding value: {all_padding_value}" if use_all_padding else f"Padding Value: {latent_padding}") + ")")
            print(f"  - Current number of generated frames: {total_generated_latent_frames * 4 - 3}フレーム")
            print(f"  - Number of frames to be generated: {num_frames}フレーム")
            print(f"  - First section?: {is_first_section}")
            print(f"  - Final section?: {is_last_section}")
            ...
            if is_first_section and end_frame_latent is not None:
                ...
                if end_frame_strength != 1.0:
                    ...
                    print(f"EndFrame influence{end_frame_strength:.2f}(The influence of the last frame is set to {end_frame_strength:.2f}）")
            ...
            if use_lora and has_lora_support and lora_file is not None:
                try:
                    ...
                    print(f"Loading LoRA: {os.path.basename(lora_path)} (Scale: {lora_scale})")
                    ...
                    print(f"We applied LoRA by DynamicSwapLoRA (スケール: {lora_scale})")
                    ...
                    try:
                        ...
                        print(f"LoRA Application Status: {has_lora}, Applicable methods: {source}")
                    except Exception as diagnostic_error:
                        print(f"LoRA diagnostic error: {diagnostic_error}")
                    
                except Exception as e:
                    print(f"LoRA application error: {e}")
                    traceback.print_exc()
                    print("LoRA deployment failed. Continue in normal mode.")
            ...
            if save_section_frames and history_pixels is not None:
                try:
                    ...
                    print(f"[WARN] Error saving last frame image in section{ i_section }: {e}")
            ...
            print(f"\u25a0 Processing for section {i_section} completed")
            print(f"  - Current cumulative frame count: {int(max(0, total_generated_latent_frames * 4 - 3))}frames")
            print(f"  - Render Time: {max(0, (total_generated_latent_frames * 4 - 3) / 30) :.2f}frames")
            ...
            if is_last_section:
                ...
                if HAS_WINSOUND:
                    ...
                    print("\n✓ Processing completed!")  # Linuxでの代替通知
                ...
                completion_message = f"All Sections({total_sections}/{total_sections})Completed. Total processing time: {time_str}"
                stream.output_queue.push(('progress', (None, completion_message, make_progress_bar_html(100, 'Processing complete'))))
                    ...
                    for file in files:
                        ...
                        if file.startswith(job_id_part) and file.endswith('.mp4') and file != final_video_name:
                            ...
                            try:
                                os.remove(file_path)
                                deleted_count += 1
                                print(f"[Delete] Intermediate files: {file}")
                            except Exception as e:
                                print(f"[Error] Error when deleting file {file}: {e}")
                    
                    if deleted_count > 0:
                        print(f"[済] {deleted_count}intermediate files deleted. Final file saved: {final_video_name}")
                        stream.output_queue.push(('progress', (None, f"{deleted_count}Intermediate files have been deleted. The final video has been saved.", make_progress_bar_html(100, '処理完了'))))
...
def process(input_image, end_frame, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, use_random_seed, save_section_frames, keep_section_videos, output_dir, section_settings, use_lora=False, lora_file=None, lora_scale=0.8, lora_format="HunyuanVideo", end_frame_strength=1.0, use_all_padding=False, all_padding_value=1.0, frame_size_setting="1秒 (33フレーム)"):
    ...
    if frame_size_setting == "0.5秒 (17フレーム)":
        ...
        print(f'Set frame size to 0.5 second mode: latent_window_size = {latent_window_size}')
    else:
        # デフォルトの1秒モードではlatent_window_size=9を使用（9*4-3=33フレーム≒1秒@30fps）
        latent_window_size = 9
        print(f'Set frame size to 1 second mode: latent_window_size = {latent_window_size}')
    ...
    print(f"\u25c6 Video length: {total_second_length}秒")
    print(f"\u25c6 Frame size: {frame_size_setting}")
    print(f"\u25c6 Number of generated sections: {total_latent_sections}回")
    print(f"\u25c6 Sampling Steps: {steps}")
    print(f"\u25c6 TeaCache enabled: {use_teacache}")
    print(f"\u25c6 LoRA enabled: {use_lora}")
    
    # オールパディング設定のログ出力
    if use_all_padding:
        print(f"\u25c6 allpadding:enabled (value: {round(all_padding_value, 1)})")
    else:
        print(f"\u25c6 All Padding: Disabled")
    
    # LoRA情報のログ出力
    if use_lora and lora_file is not None:
        print(f"\u25c6 LoRAファイル: {os.path.basename(lora_file.name)}")
        print(f"\u25c6 LoRA Scale: {lora_scale}")
        print(f"\u25c6 LoRA Format: {lora_format}")
    ...
    if valid_sections:
        print(f"\u25c6 Keyframe image to use: Section{', '.join(map(str, valid_sections))}")
    else:
        print(f"\u25c6 Keyframe Images: Use only default settings")
...
with block:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            mode_radio = gr.Radio(choices=[MODE_TYPE_NORMAL, MODE_TYPE_LOOP], value=MODE_TYPE_NORMAL, label="生成モード", info="Normal: General generation / Loop: For looping videos")
        with gr.Column(scale=1):
            ...
            frame_size_radio = gr.Radio(
                ...
                value="1秒 (33フレーム)", 
                label="セクションフレームサイズ", 
                info="1秒 = standard / 0.5秒 = smoother motion (experimental)"
            )
        with gr.Column(scale=1):
            # オールパディング設定
            use_all_padding = gr.Checkbox(label="オールパディング", value=False, info="The smaller the number, the less influence there is on the previous picture, and the more movement there is.", elem_id="all_padding_checkbox")
            all_padding_value = gr.Slider(label="パディング値", minimum=0.2, maximum=3, value=1, step=0.1, info="Padding value to apply to all sections (0.2 to 3 decimal points accepted)", visible=False)
        ...
        with gr.Column(scale=1):
            ...
            gr.Markdown("### Set your video length here first.")
            length_radio = gr.Dropdown(label="動画長", choices=get_video_modes(), value="1秒", info="Set the copy range of the keyframe image and the length of the video", type="value")
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Row():
                # プロンプト管理パネルの追加
                with gr.Accordion("Saved Prompts", open=False, elem_classes="section-accordion"):

                    with gr.Group(visible=True) as prompt_management:
                        gr.Markdown("*If you leave the preset name blank and click 'Save', it will become the default at startup.")
                        ...
                        with gr.Group(visible=True):
                            ...
                            with gr.Row():
                                edit_name = gr.Textbox(label="プリセット名", placeholder="Enter prompt name...", value=default_name)
                            ...
                            with gr.Row():
                                save_btn = gr.Button(value="Save Preset", variant="primary")
                                apply_preset_btn = gr.Button(value="Apply", variant="primary")
                                clear_btn = gr.Button(value="Clear")
                                delete_preset_btn = gr.Button(value="Delete")
            ...
            gr.Markdown("**Final is the last image, and Image is the first image (required).**")
            ...
            def generate_section_title(total_sections):
                ...
                return f"### Section Settings, in order of generation (reverse) \n\nSections are displayed in reverse chronological order. Image (start point) is required and you must set the image going backwards from Final (end point). Total number: {total_sections}"
            ...
            with gr.Accordion("Section Controls", open=False, elem_classes="section-accordion"):
                with gr.Group(elem_classes="section-container"):
                    ...
                    for i in range(max_keyframes):
                        with gr.Row(visible=(i < initial_sections_count), elem_classes="section-row") as row_group:
                            # 左側にセクション番号とプロンプトを配置
                            with gr.Column(scale=1):
                                section_number = gr.Number(label=f"Section Number{i}", value=i, precision=0)
                                section_prompt = gr.Textbox(label=f"Section prompt{i}", placeholder="Section-specific prompt (if blank, uses the main prompt)", lines=2)
                            
                            # 右側にキーフレーム画像のみ配置
                            with gr.Column(scale=2):
                                section_image = gr.Image(label=f"Keyframe Images{i}", sources="upload", type="numpy", height=200)
            ...
            with gr.Accordion("Lora Settings", open=False, elem_classes="section-accordion", visible=has_lora_support):
                                # LoRA設定グループを追加
                with gr.Group() as lora_settings_group:
                    gr.Markdown("### LoRA設定")
                    
                    # LoRA使用有無のチェックボックス
                    use_lora = gr.Checkbox(label="LoRAを使用する", value=False, info="Check to use LoRA (requires 16GB VRAM or higher)") 
                    ...
                    lora_blocks_type = gr.Dropdown(
                        ...
                        value="all",
                        info="Select block type (all=everything, other=save memory)",
                        visible=False
                    )
            ...
            if not has_lora_support:
                        gr.Markdown("LoRA support is currently disabled, the lora_utils module is required.")
                
            with gr.Row():
                # EndFrame影響度調整スライダー
                with gr.Group():
                    gr.Markdown("### EndFrame influence adjustment")
                    end_frame_strength = gr.Slider(
                        ...
                        value=1.00, 
                        step=0.01, 
                        info="Adjusts the strength of the influence of the last frame on the entire video. Lower values ​​will weaken the influence of the last frame and transition to the first frame sooner. 1.00 is normal behavior."
                    )
            ...
            with gr.Accordion("Additional Settings", open=False, elem_classes="section-accordion", visible=has_lora_support):
                with gr.Group():
                    ...
                    with gr.Row(equal_height=True):
                        with gr.Column(scale=4):
                            ...
                            output_dir = gr.Textbox(
                                ...
                                info="Folder name for saving videos and keyframe images",
                                placeholder="outputs"
                            )
                        with gr.Column(scale=1, min_width=100):
                            open_folder_btn = gr.Button(value="📂 Save and open output folder", size="sm")
                    ...
                    def handle_open_folder_btn(folder_name):
                        ...
                        if old_folder_name != folder_name:
                            ...
                            print(f"Output folder settings saved: {folder_name}")
