...
- `--enable-tensorrt` (or `FRAMEPACK_ENABLE_TENSORRT=1`) enables experimental TensorRT paths for the Hunyuan VAE decoder and SigLIP image encoder. It requires `torch-tensorrt`, a CUDA-capable GPU, and can be tuned with `FRAMEPACK_TRT_WORKSPACE_MB` / `FRAMEPACK_TRT_MAX_AUX_STREAMS`. The UI exposes a decode toggle under **Performance & Output** and falls back automatically if compilation fails.
...
- The similarity/alignment cache reuses transformer activations for highly similar inputs (e.g., adjacent frames). Toggle with `--disable-sim-cache` or `FRAMEPACK_ENABLE_SIM_CACHE`, configure the cosine threshold (`FRAMEPACK_SIM_CACHE_THRESHOLD`, default `0.9`), max consecutive skips (`FRAMEPACK_SIM_CACHE_MAX_SKIP`), cache size (`FRAMEPACK_SIM_CACHE_MAX_ENTRIES`), and optional FAISS-based lookup (`FRAMEPACK_SIM_CACHE_USE_FAISS=1`).
- xFormers acceleration can be forced via `--xformers-mode {off,standard,aggressive}` (or `FRAMEPACK_XFORMERS_MODE`). The aggressive mode enforces CUTLASS kernels, disables dropout, and prefers FP16/BF16 attention for maximum speed.
- Runtime caches (first-block, similarity, KV) and module wrappers persist automatically under `Cache/runtime_caches` / `Cache/module_cache`. Set `FRAMEPACK_RUNTIME_CACHE=0` to disable persistence or override the location via `FRAMEPACK_RUNTIME_CACHE_DIR`. Module caching stays on by default but can be disabled with `FRAMEPACK_ENABLE_MODULE_CACHE=0`.
